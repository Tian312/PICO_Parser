...
class NERModel(BaseModel):
    ...
    def run_epoch(self, train, dev, epoch):
        ...
        metrics,stats,chunk_stats= self.run_evaluate(dev)
        msg = " - ".join(["{} {:04.2f}".format(k, v)
                for k, v in metrics.items()])
        self.logger.info(msg)

        return metrics["f1"],stats,chunk_stats
    ...
    def run_evaluate(self, test):
        ...
        # calculate overall p/r/f1 for span
        new_macro = {}   
        all_correct = 0
        all_pred=0
        all_true =0
        for  tag in tags:
            all_correct += stats[tag]['n_correct']
            all_pred += stats[tag]['n_pred']
            all_true += stats[tag]['n_true']
            

        p =div_or_zero(all_correct, all_pred)
        r =div_or_zero(all_correct, all_true)

        new_macro={"p":p,"r":r,"f1":div_or_zero(2.0 * p * r, (p + r))}
        #macro_results = { metric: np.mean(list(results[metric].values())) for metric in results }
        macro_results = { metric: new_macro[metric] for metric in results}
        
        return macro_results,stats,chunk_stats
