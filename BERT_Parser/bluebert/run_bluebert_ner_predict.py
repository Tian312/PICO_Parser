...
#mytokenizer=text_tokenize.mytokenizer()
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    #    label_map[label] = i
    label2id_file = os.path.join(config.bluebert_dir, 'label2id.pkl')
    if not tf.gfile.Exists(label2id_file):
        ...
        with tf.gfile.Open(os.path.join(config.bluebert_dir, 'label2id.pkl'), 'rb') as rf:
            ...
            label_map = {key:value  for key,value  in label2id.items()}
    ...
    print ("\n====BEFORE ====",len(ntokens),len(segment_ids))
    for i,l in zip(ntokens,segment_ids):
        print(i,l)
    ...
    #print ("\n====BEFORE ====",len(input_ids),len(label_ids))
    ...
    while len(input_ids) < max_seq_length:
        ...
        # label_mask.append(0)
    print(len(input_ids))
    print ("\n====AFTER ====",len(input_ids),len(label_ids))
    #for i,l in zip(input_ids,label_ids):
    #    print(i,l)
...
def result_to_pair_for_return(predict_examples, predictions, id2label):
    """
    Args:
        predict_examples (list): InputExample, no X
        output: 
    ...
    for predict_line, pred_ids in zip(predict_examples, predictions):
        ...
        for id in pred_ids:
            ...
            pred_labels.append(curr_label)
        ''' 
        print (len(words),len(pred_labels))
        print ("\n--WORDS--")
        for w in words:
            print (w)
        print ("\n--pred_labels--")
